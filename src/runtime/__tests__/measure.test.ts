...
describe("measure()", () => {
  ...
  it("measures O(n²) quadratic function correctly", async () => {
    // Use input sizes large enough that each run exceeds the JIT/overhead
    // floor (~0.05 ms) even on fast hardware, ensuring ≥3 clean log-log
    // data points for reliable regression.
    const outcome = await measure({
