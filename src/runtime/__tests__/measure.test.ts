...
describe("measure()", () => {
  ...
  it(
    "measures O(n) linear function correctly",
    async () => {
      const outcome = await measure({
  ...
  it(
    "measures O(n²) quadratic function correctly",
    async () => {
      const outcome = await measure({
        targetPath: QUADRATIC_PATH,
        exportName: "quadraticPairs",
        generatorCode: INT_ARR_GEN,
        inputSizes: [50, 100, 200, 400],
        warmup: 1,
        trials: 3,
        timeoutMs: 20_000,
      });
  ...
  it(
    "returns timeout when the target exceeds the time limit",
    async () => {
  ...
  it(
    "returns error when the export is not a function",
    async () => {
      const outcome = await measure({
        targetPath: LINEAR_PATH,
  ...
  it(
    "reconciles as 'agree' when empirical matches static",
    async () => {
      const outcome = await measure({
        targetPath: LINEAR_PATH,
        exportName: "linearScan",
