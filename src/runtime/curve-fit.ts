 ...
 * estimate the power-law exponent (slope). Robust to per-call overhead
 * inflating small-n timings, which biases direct feature regression. Thresholds:
 *   < 0.3   → O(1)
 *   0.3–0.65 → O(log n)
 ...
 * measurement floor (all overhead-dominated).
 */
export function fitBigO(ns: number[], times: number[]): FitResult {
  ...
  // Collect points above the JIT/overhead floor (~0.05 ms). At small n,
  // V8 startup cost dominates, keeping timings flat regardless of n — those
  // points drag the log-log slope toward 0 and cause misclassification.
  const MIN_MS = 0.05;
  const valid = ns
  ...
  if (valid.length >= 2) {
    ...
    if (valid.length >= 3) {
      ...
      // Only 2 clean points — pairwise exponent is the best estimate available.
      // Assign conservative R² (not trivially 1) to signal medium confidence.
      slope = (Math.log(last.t) - Math.log(first.t)) / (Math.log(last.n) - Math.log(first.n));
      rSquared = 0.85;
    }
    ...
    if (slope < 0.3) {
      ...
      // Log-log slope ≈ 1 for both O(n) and O(n log n); use R² on raw
  ...
  // Fallback: all timings below the floor — use multi-model feature regression.
  return fitBigOByFeature(ns, times);
}
